...
快捷台的“快捷菜单”分组会通过 macOS 全局 `Fn-C` 快捷键打开控制中心，再选择
Wi-Fi、蓝牙或 AirDrop 项目，因此当前应用处于全屏 Space 时仍可使用。“切换扬声器”
会在当前可用的 Core Audio 输出设备间循环，并在手机上显示设备名称。唤醒、锁定和
显示器睡眠位于快捷台顶部，较少使用的窗口操作位于最底部。三页共用的紧凑手势提示
常驻底部导航栏下方。
